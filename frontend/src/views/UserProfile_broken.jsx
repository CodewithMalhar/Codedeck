import axios from "axios"
import { useContext, useEffect, useState } from "react"
import { useParams } from "react-router-dom"
import { AuthContext } from "../context/AuthContext"
  const apiBaseUrl = import.meta.env.VITE_API_BASE_URL; 
function UserProfile() {
    const [userData, setUserData] = useState({})
    const [portfolioData, setPortfolioData] = useState({})
    const [linkedAccounts, setLinkedAccounts] = useState({})
    const [loading, setLoading] = useState(true)
    const [error, setError] = useState(null)
    const { user_id } = useParams()
    const [showModal, setShowModal] = useState('')
        const {user}=useContext(AuthContext)
    
    const logoMap = {
        "Leetcode": "https://img.icons8.com/external-tal-revivo-shadow-tal-revivo/24/external-level-up-your-coding-skills-and-quickly-land-a-job-logo-shadow-tal-revivo.png",
        "CodingNinjas": "data:image/png;base64,",
        "Codeforces": "https://img.icons8.com/external-tal-revivo-filled-tal-revivo/24/external-codeforces-programming-competitions-and-contests-programming-community-logo-filled-tal-revivo.png",
        "GfG": "https://digitomize.com/assets/geeksforgeeks-1a83bb08.svg",
        "CodeChef": "https://digitomize.com/assets/codechef-f6a4f2da.svg",
        "GitHub": "https://img.icons8.com/ios-glyphs/30/github.png",
        "HackerRank": "https://img.icons8.com/windows/32/hackerrank.png",
    }
    const contactMap = {
        "X": {
            "img": "https://img.icons8.com/ios-filled/50/twitterx--v1.png",
            "profile": "https://twitter.com/rchandra"
        },
        "LinkedIn": {
            "img": "https://img.icons8.com/fluency/48/linkedin.png",
            "profile": "https://linkedin.com/rchandra"
        },
        "Medium": {
            "img":"https://img.icons8.com/glyph-neue/64/medium-monogram.png",
            "profile": "https://medium.com/rchandra"
        },
        "StackOverflow": {
            "img":"https://img.icons8.com/color/48/stackoverflow.png",
            "profile": "https://stackoverflow.com/rchandra"
        },
    }

    const getPlatformStats = (platform) => {
        const stats = {};
        
        if (platform === 'Leetcode' && portfolioData.lcStats) {
            const lc = portfolioData.lcStats;
            stats.Username = lc.profile?.username || linkedAccounts.lcUsername || 'N/A';
            stats.Ranking = lc.profile?.ranking || 'N/A';
            stats['Problems Solved'] = lc.solved?.totalsolved || 'N/A';
            stats['Easy Solved'] = lc.solved?.easy || 'N/A';
            stats['Medium Solved'] = lc.solved?.medium || 'N/A';
            stats['Hard Solved'] = lc.solved?.hard || 'N/A';
            stats['Contest Rating'] = lc.contest?.contestRating || 'N/A';
            stats['Contests Attended'] = lc.contest?.totalattended || 'N/A';
            stats['Global Ranking'] = lc.contest?.contestGlobalRanking || 'N/A';
            stats['Profile Link'] = `https://leetcode.com/${linkedAccounts.lcUsername}`;
        } else if (platform === 'Codeforces' && portfolioData.cfStats) {
            const cf = portfolioData.cfStats;
            stats.Username = cf.profile?.handle || linkedAccounts.cfUsername || 'N/A';
            stats['Current Rating'] = cf.profile?.currentRating || 'N/A';
            stats['Max Rating'] = cf.profile?.maxRating || 'N/A';
            stats.Rank = cf.profile?.rank || 'N/A';
            stats['Max Rank'] = cf.profile?.maxRank || 'N/A';
            stats['Problems Solved'] = cf.solved?.totalSolved || 'N/A';
            stats['Contests Participated'] = cf.contest?.contestsParticipated || 'N/A';
            stats['Profile Link'] = `https://codeforces.com/profile/${linkedAccounts.cfUsername}`;
        } else if (platform === 'GfG' && linkedAccounts.gfgUsername) {
            stats.Username = linkedAccounts.gfgUsername;
            stats['Profile Link'] = `https://auth.geeksforgeeks.org/user/${linkedAccounts.gfgUsername}`;
            stats.Status = 'API not integrated yet';
        } else if (platform === 'CodeChef' && linkedAccounts.ccUsername) {
            stats.Username = linkedAccounts.ccUsername;
            stats['Profile Link'] = `https://www.codechef.com/users/${linkedAccounts.ccUsername}`;
            stats.Status = 'API not integrated yet';
        } else if (platform === 'GitHub' && linkedAccounts.githubUsername) {
            stats.Username = linkedAccounts.githubUsername;
            stats['Profile Link'] = `https://github.com/${linkedAccounts.githubUsername}`;
            stats.Status = 'API not integrated yet';
        } else if (platform === 'HackerRank' && linkedAccounts.hrUsername) {
            stats.Username = linkedAccounts.hrUsername;
            stats['Profile Link'] = `https://www.hackerrank.com/profile/${linkedAccounts.hrUsername}`;
            stats.Status = 'API not integrated yet';
        } else if (platform === 'CodingNinjas' && linkedAccounts.cnUsername) {
            stats.Username = linkedAccounts.cnUsername;
            stats['Profile Link'] = `https://www.codingninjas.com/students/profile/${linkedAccounts.cnUsername}`;
            stats.Status = 'API not integrated yet';
        } else {
            return 'No stats available';
        }
        
        return stats;
    };
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                setError(null);
                
                // Fetch user data
                const userRes = await axios.get(`${apiBaseUrl}/users/getUser`, {
                    params: { user_id }
                });
                setUserData(userRes.data);
                
                // Fetch linked accounts
                const linkedRes = await axios.get(`${apiBaseUrl}/portfolio/linkedaccounts`, {
                    params: { _id: user_id }
                });
                setLinkedAccounts(linkedRes.data);
                
                // Fetch portfolio stats
                try {
                    const portfolioRes = await axios.put(`${apiBaseUrl}/portfolio/getupdateportfolio`, null, {
                        params: { _id: user_id }
                    });
                    setPortfolioData(portfolioRes.data);
                } catch (portfolioError) {
                    console.log('Portfolio data not available:', portfolioError);
                    // Continue without portfolio data
                }
                
            } catch (error) {
                console.error('Error fetching user data:', error);
                setError('Failed to load user data');
            } finally {
                setLoading(false);
            }
        };
        
        if (user_id) {
            fetchData();
        }
    }, [user_id])
    useEffect(() => {
        if(!user){
            return
        }
        console.log(user._id)
        console.log(user_id)
        if(String(user._id)===String(user_id)){
            console.log("same same")
            return
        }
        const updateView = async () => {
            try {
                const res = await axios.post(`${apiBaseUrl}/users/updateViews`, {
                    _id:user?._id,
                    userId:user_id
                })
                console.log(res.data);
            } catch (error) {
                console.log(error);
            }
        }
        updateView()

    }, [user_id])
    console.log(user_id);
    if (!user_id) {
        return <h1 className="text-white text-center mt-20">No user found</h1>
    }
    
    if (loading) {
        return <h1 className="text-white text-center mt-20">Loading profile...</h1>
    }
    
    if (error) {
        return <h1 className="text-red-500 text-center mt-20">{error}</h1>
    }
    
    if (Object.keys(userData).length === 0) {
        return <h1 className="text-white text-center mt-20">User not found</h1>
    }
    else {
		return (
			<div className="mt-10">
				<section className="w-full flex flex-col gap-2 pt-20  ">
					<div className="grid grid-cols-2 p-16 w-[90%] mx-auto bg-[#161616]">
						<div className="info col-span-1 bg-[#1B1B1B] shadow-lg p-10">
							<h1 className="text-5xl text-start text-white font-extrabold">{userData?.username}</h1>
							<p className="text-xl mb-2 text-start text-white font-[100]  font-sans">{userData?.firstname} {userData?.lastname}</p>
							{
								userData?.bio && <p className="text-white text-md  font-[100] font-sans">
									Description
									<span className="text-[#5bdac7e7]"> {userData.bio}</span>
								</p>
							}
							{
								userData?.institute && (
									<p className="text-white text-md  font-sans font-[100]">
										Institute name
										<span className="text-[#5bdac7e7]"> {userData.institute}</span>
									</p>
								)
							}
							<p className="text-white text-md  font-sans font-[100]">
								Contests attended
								<span className="text-[#5bdac7e7]"> 
                                    {portfolioData.cfStats?.contest?.contestsParticipated || 
                                     portfolioData.lcStats?.contest?.totalattended || 
                                     'N/A'}
                                </span>
							</p>
							<p className="text-white text-2xl font-bold font-sans mt-2">
								Available profiles
							</p>
							<div className="flex flex-wrap gap-3 mt-2">

								{
									Object.keys(logoMap).map((keys, index) => (
										<div className="relative"
											key={index}
											onMouseEnter={() => setShowModal(keys)}
											onMouseLeave={() => setShowModal('')}
										>
											<img
												src={logoMap[keys]} className="h-14 w-14 cursor-pointer hover:scale-125 transition-all  rounded-full p-2"
											/>
											{
												showModal === keys && <StatsModal keys={keys} stats={getPlatformStats(keys)} />
											}

										</div>
									))
								}
							</div>
							<p className="text-white text-2xl font-bold font-sans mt-2">
								Contacts
							</p>
							<div className="flex flex-wrap gap-3 mt-2">

								{
									Object.keys(contactMap).map((keys, index) => (
										<a className="relative"
											key={index}
											href={contactMap[keys].profile}
											target="_blank"
										>
											<img
												src={contactMap[keys].img} className="h-14 w-14 cursor-pointer hover:scale-125 transition-all  rounded-full p-2"
											/>
										</a>
									))
								}
							</div>





						</div>

						<div className="profilepic relative col-span-1 flex p-10 shadow-lg justify-center bg-[#1B1B1B]">
							<div className="bg-[#5bdac7e7] absolute h-full  z-10 right-0 top-0 w-full"
								style={{ clipPath: "polygon(66% 0, 100% 0, 100% 100%, 25% 100%, 83% 49%)" }}
							>

							</div>
							<img src={userData.profileUrl} alt="profile" className="shadow-lg w-96 h-96 object-cover z-20" />
						</div>
					</div>

				</section>
			</div>
		)
    }
}

const StatsModal = ({ keys, stats }) => {
    return (
        <div className={`absolute left-0 z-30 pt-7 h-auto ${stats === "No stats available" ? '-bottom-16' : '-bottom-36'}`}>
            <div className="p-2 bg-slate-800 w-60 text-white rounded-lg px-4 py-4">
                <div className="title text-white font-semibold">
                </div>
            )
        }
    }

    const StatsModal = ({ keys, stats }) => {
        return (
            <div className={`absolute left-0 z-30 pt-7 h-auto ${stats === "No stats available" ? '-bottom-16' : '-bottom-36'}`}>
                <div className="p-2 bg-slate-800 w-60 text-white rounded-lg px-4 py-4">
                    <div className="title text-white font-semibold">
                                return (
                                    <a key={index} href={stats[key]} target="_blank" rel="noreferrer" className="text-[#5bdac7e7] block mb-1">
                                        {key}
                                    </a>
                                )
                            }
                            return (
                                <div key={index} className="flex justify-between items-center mb-1">
                                    <p className="text-sm">{key}</p>
                                    <p className="text-sm font-semibold">{stats[key]}</p>
                                </div>
                            )
                        })
                    )}
                </div>
            </div>
        </div>
    )
}

export default UserProfile